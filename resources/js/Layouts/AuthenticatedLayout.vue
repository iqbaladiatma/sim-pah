    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col lg:flex-row overflow-x-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-screen bg-gray-50/50 dark:bg-gray-900">
            ...
            <header class="h-20 bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl border-b border-gray-100 dark:border-gray-700/50 items-center justify-between px-4 sm:px-6 lg:px-12 sticky top-0 z-40 flex shadow-[0_1px_40px_-10px_rgba(0,0,0,0.05)]">
                ...
                <div class="flex items-center gap-6">
                    <Dropdown align="right" width="64" contentClasses="py-2 bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl border border-gray-100 dark:border-gray-700/50 shadow-2xl rounded-2xl overflow-hidden ring-0">
                        <template #trigger>
                            <button class="flex items-center gap-3 transition-all hover:scale-105 active:scale-95 group">
                                <div class="hidden md:flex flex-col items-end">
                                    <span class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-tighter">{{ user.name }}</span>
                                    <span class="text-[9px] font-black text-pail-gold uppercase tracking-widest opacity-80">{{ user.role }}</span>
                                </div>
                                <div class="w-10 h-10 rounded-2xl bg-pail-gold/10 text-pail-gold flex items-center justify-center font-black shadow-inner border border-pail-gold/20 group-hover:bg-pail-gold group-hover:text-white transition-all duration-500 transform group-hover:rotate-6">
                                    {{ user.name.charAt(0) }}
                                </div>
                            </button>
                        </template>
                        <template #content>
                            <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700/50 bg-gray-50/50 dark:bg-gray-900/50">
                                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Masuk sebagai</p>
                                <p class="text-[11px] font-black text-gray-900 dark:text-white truncate tracking-tighter">{{ user.email }}</p>
                            </div>
                            
                            <div class="p-2">
                                <DropdownLink :href="route('profile.edit')" class="rounded-xl transition-all duration-300">
                                    <div class="flex items-center gap-4 py-2 px-1 group">
                                        <div class="w-8 h-8 rounded-lg bg-gray-50 dark:bg-gray-900 flex items-center justify-center text-gray-400 group-hover:text-pail-gold group-hover:bg-pail-gold/10 transition-all border border-transparent group-hover:border-pail-gold/20">
                                            <SettingsIcon class="w-4 h-4 transition-transform group-hover:rotate-45" />
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-[11px] font-black text-gray-700 dark:text-gray-200 uppercase tracking-tight">Konfigurasi Profil</span>
                                            <span class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Pengaturan akun anda</span>
                                        </div>
                                    </div>
                                </DropdownLink>
                                ...
                                <DropdownLink :href="route('logout')" method="post" as="button" class="w-full text-left rounded-xl transition-all duration-300">
                                    <div class="flex items-center gap-4 py-2 px-1 group">
                                        <div class="w-8 h-8 rounded-lg bg-red-50 dark:bg-red-900/20 flex items-center justify-center text-red-400 group-hover:text-white group-hover:bg-red-500 transition-all shadow-sm">
                                            <LogOutIcon class="w-4 h-4" />
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-[11px] font-black text-red-600 dark:text-red-400 uppercase tracking-tight">Keluar Sistem</span>
                                            <span class="text-[8px] font-bold text-red-300 dark:text-red-900/40 uppercase tracking-widest mt-0.5">Selesaikan sesi aktif</span>
                                        </div>
                                    </div>
                                </DropdownLink>
                            </div>
            ...
            <footer class="shrink-0 px-4 sm:px-8 lg:px-14 py-8 pb-32 lg:pb-8 border-t border-gray-100 dark:border-gray-700/50 bg-white/30 dark:bg-gray-800/30 backdrop-blur-sm">
                <div class="flex flex-col lg:flex-row items-center justify-between gap-4 lg:gap-8">
                    <div class="flex items-center gap-4">
                        <p class="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-tight">
                            © {{ new Date().getFullYear() }} <span class="hidden sm:inline">Pondok Pesantren Abu Hurairah Mataram</span><span class="sm:hidden">PP Abu Hurairah</span>
                        </p>
                        <div class="w-px h-3 bg-gray-200 dark:bg-gray-700 hidden sm:block"></div>
                        <p class="text-[8px] font-black text-pail-gold uppercase tracking-widest leading-none hidden sm:block">
                            SIM - URT
                        </p>
                    </div>
                    
                    <div class="flex items-center gap-4 bg-gray-50/50 dark:bg-gray-900/30 px-5 py-2 rounded-xl border border-gray-100 dark:border-gray-700/50 shadow-inner">
                        <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Dev by</span>
                        <div class="flex items-center gap-3">
                            <a href="https://instagram.com/iq_html" target="_blank" class="group flex items-center gap-1.5 transition-all">
                                <div class="w-5 h-5 rounded-lg bg-pink-500/10 flex items-center justify-center text-pink-500 group-hover:bg-pink-500 group-hover:text-white transition-all">
                                ...
                                <span class="text-[9px] font-bold text-gray-500 group-hover:text-pink-600 transition-colors tracking-tighter">@iq_html</span>
                            </a>
                            ...
                            <a href="https://linkedin.com/in/Iqbaladiatma" target="_blank" class="group flex items-center gap-1.5 transition-all">
                                ...
                                <span class="text-[9px] font-bold text-gray-500 group-hover:text-blue-600 transition-colors tracking-tighter">@Iqbaladiatma</span>
                            </a>
                        </div>
                    </div>
                </div>
